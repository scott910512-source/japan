...
ok('학습 · 퀴즈 · 기타',
  MENU_GROUPS.map((g) => g.label).join() === '학습,퀴즈,기타');
...
ok('단어암기는 남아 있다', MENU_IDS.includes('words'));
...
ok('묶음 순서가 지켜진다', g.map((x) => x.id).join() === 'learn,quiz,etc');
ok('칸 수가 맞는다', g.reduce((a, x) => a + x.items.length, 0) === MENUS.length);

const some = groupedMenus({ translate: true });
ok('빈 묶음은 안 그린다', some.length === 1 && some[0].id === 'etc',
