...
console.log('\n── 연도별');
{
  ...
  /* 한자읽기는 한 회에 여덟 문제, 한 해에 두 회라 한 해에 열여섯 개다.
     빠지는 해가 둘 있고, 이유가 서로 다르다.

       2020 — 8개. 그해 7월 시험이 취소되어 한 회만 치렀다. 자료가 맞다.
       2022 — 15개. ★ 옮겨 적을 때 한 줄이 빠졌다 ★ 교재 사진에서 그 줄이
              잘려 안 보인다. 채워지면 16으로 바꾼다.

     둘을 한 줄에 눈감지 않고 따로 적는 이유가 이것이다. 「16이 아닌 해가
     둘 있다」로 두면 다음에 한 해가 통째로 잘못 들어와도 이 검사가 조용하다. */
  const byYear = Object.fromEntries(years.map((y) => [y.year, y.items.length]));
  ok('2020년은 여덟 개 — 7월 시험 취소', byYear[2020] === 8, `${byYear[2020]}개`);
  ok('2022년은 아직 열다섯 개 — 사진에서 한 줄이 잘렸다', byYear[2022] === 15, `${byYear[2022]}개`);
